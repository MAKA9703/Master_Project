...
test_query = test_query.sort_values(by = ['qid'])
...
def retrieve_res(df, docs):
    comb = pd.merge(df, docs, on='docno')
    comb['rank'] = comb['rank'].astype('int64')
    comb = comb.sort_values(by = ['qid', 'rank'])
    new_comb = comb[['qid', 'docno', 'text']].reset_index(drop=True)
    qids = new_comb['qid'].unique()

    passages = []
    for i in range(len(qids)):
        passages.append(new_comb[new_comb['qid']== qids[i]][['docno','text']].values.tolist())
