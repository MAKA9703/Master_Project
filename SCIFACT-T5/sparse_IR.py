...
df_test2.to_csv('./scifact/temp_test_queries.csv', sep = '\t', index=False, header=False)
test_query = pt.io.read_topics('./scifact/temp_test_queries.csv', format='singleline')
...
def eval_sparse(query, name):
    ...
    res.to_csv(f'results/sparse_ir_res_{name}.txt',  sep='\t', index=False)



eval_sparse(test_alt1, "alt1")

print("1 out of 5 done!")

eval_sparse(test_alt3, "alt3")

print("2 out of 5 done!")

eval_sparse(test_comb1, "comb1")

print("3 out of 5 done!")

eval_sparse(test_comb2, "comb2")

print("4 out of 5 done!")

eval_sparse(test_comb3, "comb3")
