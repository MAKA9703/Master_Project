...
train_q2 =  pt.io.read_topics('./scifact/rephrase/T5_train_query2.csv', format='singleline')
train_q3 =  pt.io.read_topics('./scifact/rephrase/T5_train_query3.csv', format='singleline')


train_q4 = train_q
train_q4['query'] = train_q4['query'] + " " + train_q1['query'] + " " + train_q2['query'] + " " + train_q3['query']
...
train_samples = retriever.load_train(new_corpus, dict_tr_q4, new_train_qrels)
...
model_save_path = os.path.join(pathlib.Path('./dpr_models').parent.absolute(), "dpr_models/output_comb", "{}-v1-{}".format(model_name, "SCI-FACT"))
